            ...
            for i in range(45):
                if response.status_code == 200 and data_json["txs"][0]["data"]["code"] == 0:
                    ...
                    if txhash not in txSet:
                        txSet.add(txhash)
                        timestamp = data_json["txs"][i]["header"]["timestamp"]
                        jsonString = data_json["txs"][i]["data"]["raw_log"]
                        aDict = json.loads(jsonString)
                        bDict = json.loads(aDict[1]["events"][1]["attributes"][0]["value"])
                        amount = bDict["amount"]
                        senderAddress = bDict["sender"]
                        receiverAddress = bDict["receiver"]
                        print(amount)
                        print(senderAddress)
                        print(receiverAddress)
                        # jsonString = data_json["txs"][i]["data"]["logs"][1]["events"][1]["attributes"][0]["value"]
                        # aDict = json.loads(jsonString)
                        # amount = aDict["amount"]
